...
export class ArchiveProjectUseCase implements IArchiveProjectUseCase {
  ...
  constructor(
    ...
    this.logger = logger.child("ArchiveProjectUseCase");
  }

  async execute(projectId: string, requesterId: string): Promise<Project> {
    try {
      this.logger.debug("Archiving project", { projectId, requesterId });
      ...
      if (!this.isOwnerOrAdmin(team, requesterId)) {
        this.logger.warn("Unauthorized project archive attempt", {
          projectId,
          requesterId,
        });
      ...
      this.logger.info("Project archived", { projectId });
      return updated;
    } catch (error: any) {
      ...
      throw new ApplicationError("Could not archive project", { cause: error });
    }
  }
