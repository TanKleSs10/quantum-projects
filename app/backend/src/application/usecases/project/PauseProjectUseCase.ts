...
export interface IPauseProjectUseCase {
  execute(projectId: string, requesterId: string): Promise<Project>;
}

export class PauseProjectUseCase implements IPauseProjectUseCase {
  ...
  constructor(
    ...
    this.logger = logger.child("PauseProjectUseCase");
  }

  async execute(projectId: string, requesterId: string): Promise<Project> {
    try {
      this.logger.debug("Pausing project", { projectId, requesterId });
      ...
      if (!this.isOwnerOrAdmin(team, requesterId)) {
        this.logger.warn("Unauthorized project pause attempt", {
          projectId,
          requesterId,
        });
      ...
      this.logger.error("Failed to pause project", {
        projectId,
        error: error instanceof Error ? error.message : String(error),
      });

      throw new ApplicationError("Could not pause project", { cause: error });
    }
  }
