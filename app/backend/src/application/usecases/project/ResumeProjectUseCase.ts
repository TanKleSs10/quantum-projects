...
export interface IResumeProjectUseCase {
  execute(projectId: string, requesterId: string): Promise<Project>;
}

export class ResumeProjectUseCase implements IResumeProjectUseCase {
  ...
  constructor(
    ...
    this.logger = logger.child("ResumeProjectUseCase");
  }

  async execute(projectId: string, requesterId: string): Promise<Project> {
    try {
      this.logger.debug("Resuming project", { projectId, requesterId });
      ...
      if (!this.isOwnerOrAdmin(team, requesterId)) {
        this.logger.warn("Unauthorized project resume attempt", {
          projectId,
          requesterId,
        });
      ...
      this.logger.error("Failed to resume project", {
        projectId,
        error: error instanceof Error ? error.message : String(error),
      });

      throw new ApplicationError("Could not resume project", { cause: error });
    }
  }
