...
export class DeleteProjectUseCase implements IDeleteProjectUseCase {
  ...
  constructor(
    ...
    this.logger = logger.child("DeleteProjectUseCase");
  }

  async execute(projectId: string, requesterId: string): Promise<void> {
    try {
      this.logger.debug("Deleting project", { projectId, requesterId });
      ...
      if (!this.isOwnerOrAdmin(team, requesterId)) {
        this.logger.warn("Unauthorized project delete attempt", {
          projectId,
          requesterId,
        });
      ...
      throw new ApplicationError("Could not delete project", { cause: error });
    }
  }
